...
// images/big/2.webp
export const wb_img_url = (
  nm: number,
  imgPart: string = "/images/c246x328/1.webp"
) => {
  const vol = ~~(nm / 1e5);
  const part = ~~(nm / 1e3);
  let host = "https://";
...
export const categoryProcessor = (
  data: ResponseWbCategory,
  ...
  const items: WbResponseItem[] = (data.data.products ?? []).map(
    ({ id, name, salePriceU, priceU }) => ({
      skuId: id.toString(),
      title: name,
